...
export default function AdminProdcutsScreen() {
  ...
  const createHandler = async () => {
    if (!window.confirm('Desea crear un nuevo servicio?')) {
      return;
    }
    try {
  ...
  const deleteHandler = async (productId) => {
    if (!window.confirm('Seguro que desea eliminar el servicio?')) {
      return;
    }
    try {
      ...
      <div className="grid md:grid-cols-4 md:gap-5">
        ...
        <div className="overflow-x-auto md:col-span-3">
          <div className="flex justify-between">
            <h1 className="mb-4 text-xl">Servicios</h1>
                    ...
                    <tr key={product._id} className="border-b">
                      ...
                      <td className=" p-5 ">
                        <Link href={`/admin/product/${product._id}`}>
                          <a type="button" className="default-button hover:bg-amber-400">
                            Editar
                          </a>
                        </Link>
                        &nbsp;
                        <button
                          ...
                          className="default-button hover:bg-red-400"
                          type="button"
                        >
                          Eliminar
                        </button>
